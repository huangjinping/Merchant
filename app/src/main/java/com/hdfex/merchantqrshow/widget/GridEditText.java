...
public class GridEditText extends EditText {
	...
	public GridEditText(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.GridEditText, 0, 0);
		try {
			borderColor = a.getColor(R.styleable.GridEditText_borderColor, defaultBorderColor);
			borderWidth = a.getDimension(R.styleable.GridEditText_gborderWidth, defaultBorderWidth);
			borderRadius = a.getDimension(R.styleable.GridEditText_borderRadius, defaultBorderRadius);
			multiLength = a.getInt(R.styleable.GridEditText_multiLength, defaultMultiLength);
			multiColor = a.getColor(R.styleable.GridEditText_multiColor, defaultMultiColor);
			multiWidth = a.getDimension(R.styleable.GridEditText_multiWidth, defaultMultiWidth);
			multiRadius = a.getDimension(R.styleable.GridEditText_multiRadius, defaultMultiRadius);
			isTextVisible = a.getBoolean(R.styleable.GridEditText_multiTextVisible, true);
		} finally {
			a.recycle();
		}
...
//			}
			for (int i = 0; i < multiLength; i++) {
				...
				if (i<textLength){
					...
					canvas.drawText("-", rectCell.centerX(), baseline, paint);
				}
			}
